...
inline void init_main_settings(const std::filesystem::path& config_file) {
    ...
    SETTINGS["input_tick_rate"] =      Setting(load_setting<int>(config_file, "Ticking", "input_tick_rate", 1));
    SETTINGS["animation_frame_rate"] = Setting(load_setting<int>(config_file, "Ticking", "animation_frame_rate", 30));
    // Mulithreading //
