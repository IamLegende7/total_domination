...
// Local
std::tuple<std::string, std::string> get_compiled_path(const std::filesystem::path& hlsl_path, const SDL_GPUShaderFormat format) {
    ...
    if (hlsl_path.extension() != ".hlsl") {
        const std::string hlsl_path_str = hlsl_path.u8string();
        LOG(LogLevel::Warning, "Path \"%s\" does not point to a .hlsl file.", hlsl_path_str.c_str());
        return {"", ""};
    }
    std::string base = hlsl_path.stem().string();
...
SDL_GPUShader* load_shader(SDL_GPUDevice *device, const std::filesystem::path& filename, SDL_ShaderCross_ShaderStage stage) {
    ...
    LOG(LogLevel::Info, "Loading shader \"%s\"..", filename_str.c_str());
    SDL_GPUShader* shader;
    if (RENDER_SETTINGS["online_shaders"].get<bool>()) { 
        if (!STATUS_SHADERCROSS_LOADED) {
            LOG(LogLevel::Warning, "Could not load shader %s: SDL_shadercross not loaded.", filename_str.c_str());
            return nullptr;
        }

        size_t sz = 0;
        char *source = (char *)SDL_LoadFile(filename_str.c_str(), &sz);
        if (source == nullptr) {
            LOG(LogLevel::Error, "Could not load shader source from file \"%s\": %s", filename_str.c_str(), SDL_GetError());
            return nullptr;
        }
...
bool add_renderer_render_state(SDL_Renderer* renderer, const std::filesystem::path& filename,  std::vector<RenderState>& render_states) {
    ...
    render_states.back().name = filename_str;
...
int set_render_state(SDL_Renderer* renderer, const std::filesystem::path& filename, std::vector<RenderState>& render_states) {
    const std::string filename_str = filename.u8string();
    if (filename_str == "none") {
        return 0;
    }
    const int existing_shader_state_index = get_render_state(render_states, filename_str);
    if (existing_shader_state_index != -1) {
        //LOG(LogLevel::Warning, "Tried to add render state \"%s\" but it already exists.", filename_str.c_str());
