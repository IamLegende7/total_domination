...
bool RenderAgent::add_font(const std::string& font_name, const std::filesystem::path& font_path, const float font_size) {
    ...
    fonts[font_name] = TTF_OpenFont(font_path_str.c_str(), font_size);
    LOG(LogLevel::Info, "Added new font \"%s\" from file \"%s\".", font_name.c_str(), font_path_str.c_str());
    return true;
}
