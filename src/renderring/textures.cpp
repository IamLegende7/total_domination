...
std::filesystem::path get_texture_path(const std::string& name) {
    ...
    if (!textures_json.IsObject()) {
        LOG(LogLevel::Warning, "%s is not a valid textures.json file: root is not an object!", LOCATIONS["textures_json"].c_str());
        return LOCATIONS["missing_texture"].get<std::filesystem::path>();
    }
    if (!textures_json.HasMember("data")) {
        LOG(LogLevel::Warning, "%s is not a valid textures.json file: it does not contain value 'data'", LOCATIONS["textures_json"].c_str());
        return LOCATIONS["missing_texture"].get<std::filesystem::path>();
    }
    if (!textures_json["data"].IsObject()) {
        LOG(LogLevel::Warning, "%s is not a valid textures.json file: 'data' is not an object", LOCATIONS["textures_json"].c_str());
        return LOCATIONS["missing_texture"].get<std::filesystem::path>();
    }
    if (!textures_json["data"].HasMember(name.c_str())) {
        LOG(LogLevel::Warning, "Could not get texture of %s: %s does not contain value '%s'", name.c_str(), LOCATIONS["textures_json"].c_str(), name.c_str());
        return LOCATIONS["missing_texture"].get<std::filesystem::path>();
    }
    ...
    return replace_locations(std::filesystem::path(texture_path_raw));
};

bool add_texture(RenderAgent* agent, const std::string& texture_name) {
    std::filesystem::path texture_path = get_texture_path(texture_name);
...
bool bake_atlas(RenderAgent* agent, const std::string& atlas_name, const std::string* texture_names, const int& texture_names_length, const bool force_file_loading) {
    ...
    for (int i = 0; i < texture_names_length; ++i) {
        ...
        std::filesystem::path current_texture_path = (agent->texture_exists(current_texture_name) & !force_file_loading) ? std::filesystem::path("") : get_texture_path(current_texture_name);
    ...
    if (DEBUG["save_texture_atlases"].get<bool>()) {
        if (RENDER_SETTINGS["render_mode"].get<int>() == 1) {
            ...
            const std::filesystem::path atlas_file_path = LOCATIONS["log_dir"].get<std::filesystem::path>() / "atlas-'" / std::filesystem::path(atlas_name) / "'.png";
