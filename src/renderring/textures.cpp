...
std::string get_texture(const std::string& name) {
    ...
    //if (!textures_json["data"][name.c_str()].IsObject()) {
...
bool bake_atlas(RenderAgent* agent, const std::string& atlas_name, const std::string* texture_names, const int& texture_names_length) {
    ...
    for (int i = 0; i < texture_names_length; ++i) {
        ...
        LOG(LogLevel::DEBUG, "current_texture_path: %s", current_texture_path.c_str());
        ...
        int current_y = 0;
        while (!found_pos) {
            LOG(LogLevel::DEBUG, "Current Texture %s: %d|%d :: %d|%d", current_texture_name.c_str(), rows_x[current_row], rows_y[current_row], atlas_width, atlas_height);
            if ((current_texture.height != rows_y[current_row]) and (rows_y[current_row] != 0)) {
                current_row++;
                current_y += rows_y[current_row];
                continue;
            }
            if (rows_x[current_row]+current_texture.width <= atlas_width) { // Basic: found position
                constructors[i]->x = rows_x[current_row];
                constructors[i]->y = rows_y[current_row];
    ...
    LOG(LogLevel::DEBUG, "UwU");
    agent->insert_texture(atlas_name, agent->bake_texture_from_file(constructors, texture_names_length));
    LOG(LogLevel::DEBUG, "Baking is done!");
    return true;
}
